...
public class EnhanceScrollView2 : MonoBehaviour, IDragArenaEventListerner
{
    ...
    public int overrideCenterIdx = -1;
    ...
    void Start()
    {
        ...
        if(overrideCenterIdx > -1 && overrideCenterIdx < listEnhanceItems.Count)
        {
            startCenterIndex = overrideCenterIdx;
        }
    ...
    {
        var buffer = System.Buffers.MemoryPool<byte>.Shared;
    ...
    public Vector2 deltaScr;
    public Vector2 deltaLocal;

    // On Drag Move
    public void OnDrag(PointerEventData eventData, Vector2 deltaLocal)
    {
        deltaScr = eventData.delta;
        this.deltaLocal = deltaLocal;

        // In developing
        if (Mathf.Abs(eventData.delta.x) > 0.0f)
        {
            curHorizontalValue += deltaScr.x /totalHorizontalWidth ;
            LerpTweenToTarget(0.0f, curHorizontalValue, false);
        }

    }

    public bool PositionCenterRange(AnimationCurve curve, float f)
    {
        var points = curve.keys;
        if (points.Length > 3)
        {
            var postFirst = points[1];
            var preLast = points[points.Length - 2];
            f = Mathf.Repeat(f, 1f);
            return f >= postFirst.time && f <= preLast.time;
        }
        return false;
    }
    ...
    public void OnEndDrag(PointerEventData eventData)
    {
        ...
        {
            var rangeValue = curHorizontalValue + listEnhanceItems[i].CenterOffSet;

            if (!PositionCenterRange(positionCurve, rangeValue))
            {
                continue;
            }
