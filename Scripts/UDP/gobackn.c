// using  reliable from : https://github.com/mas-bandwidth/reliable
...
#define MAX_BUFFER_SIZE 1024 // Receiver buffer size
#define RESEND_TIMEOUT 1.0  // Seconds to wait before resending

// Structure for sender's in-flight window
...
// Structure for receiver's packet buffer
struct receiver_packet {
    uint16_t sequence;
    uint8_t* data;
    int size;
    int valid; // 1 if packet is present, 0 if slot is empty
};
...
static void sender_transmit_packet(void* context, uint64_t id, uint16_t sequence, uint8_t* packet_data, int packet_bytes) {
    ...
    reliable_endpoint_receive_packet(ctx->receiver, packet_data, packet_bytes); // Simulate direct delivery
}

// Receiver transmit function (for acks)
static void receiver_transmit_packet(void* context, uint64_t id, uint16_t sequence, uint8_t* packet_data, int packet_bytes) {
    struct context* ctx = (struct context*)context;
    reliable_endpoint_receive_packet(ctx->sender, packet_data, packet_bytes); // Simulate ack delivery
}

// Sender process packet (handle acks)
static int sender_process_packet(void* context, uint64_t id, uint16_t sequence, uint8_t* packet_data, int packet_bytes) {
    return 1; // Always ack received packets
}

// Receiver process packet (buffer and deliver in order)
...
// Send a packet if window allows
...
void process_acks(struct context* ctx) {
    ...
    for (int i = 0; i < num_acks; i++) {
        ...
        // Slide window if oldest packet is acked
        while (ctx->sender_in_flight > 0 && ctx->sender_window[0].acked) {
            free(ctx->sender_window[0].data);
            for (int k = 0; k < ctx->sender_in_flight - 1; k++) {
